...
ISO: clone → `/root/dotfiles`, run `arch-install.sh` (not bare `archinstall`). In gum: confirm the install disk (not the live USB) and LUKS. Ctrl+C on the confirm toggles encryption.
