...
| Live USB overlay (`gum`, network gate, auto-start curl) | `bin/arch-install/mkiso.sh` + `iso/` — official ISO still works without this |
...
- Live-ISO entry: `bin/arch-install/arch-install.sh` (`network.sh` then `mirrors.sh` then `configure.sh` then archinstall)
- Archinstall JSON: generated by `bin/arch-install/configure.sh`, gitignored
- Custom ISO: `bin/arch-install/mkiso.sh` (overlay in `bin/arch-install/iso/`)
